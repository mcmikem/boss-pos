...
app.post('/api/sales', (req, res) => {
  ...
  for (const item of s.items) {
    ...
    if (prod && !prod.isService) {
      db.prepare('UPDATE products SET stockQty = MAX(0, stockQty - ?) WHERE id=?').run(item.qty, item.productId);
    }
  }
  res.json(s);
});
...
app.delete('/api/sales/:id', (req, res) => {
  ...
  for (const item of items) {
    const prod = db.prepare('SELECT * FROM products WHERE id=?').get(item.productId);
    if (prod && !prod.isService) {
      db.prepare('UPDATE products SET stockQty = stockQty + ? WHERE id=?').run(item.qty, item.productId);
    }
  }
