...
export function printDailyClose(dateStr: string, sales: Sale[], expenses: Expense[], products: Product[]) {
  ...
  <table><tr><th>Revenue</th><th>VAT inside</th><th>COGS</th><th>Gross</th><th>Expenses</th><th>Net</th></tr><tr><td>${revenue.toLocaleString()}</td><td>${vat.toLocaleString()}</td><td>${cogs.toLocaleString()}</td><td>${gross.toLocaleString()}</td><td>${expTotal.toLocaleString()}</td><td><b>${net.toLocaleString()}</b></td></tr></table>
